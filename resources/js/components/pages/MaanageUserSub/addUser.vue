<template>
    <div class="h-fit flex items-center justify-center">
        <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 p-2 z-50">
            <div class="w-screen sm:w-max  md:w-max   bg-white rounded-xl">
            <div class="  ">
                <div class="flex justify-between bg-gray-500 p-3 ">
                    <h1 class="text-xl font-bold italic text-white">{{ `${title} USER`  }}</h1>
                ...
                <div class="p-4 overflow-y-auto  h-96   md:h-max  lg:h-max">
                <div >
                ...
                <div class="flex  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    <div >
                ...
                <div class="flex grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    <div >
                        <label class="block text-gray-700 text-sm font-bold mb-2">Phone No:</label>
                        <input v-model="userObj.Phone_No" @input="handlePhoneInput" type="text" class="w-full border border-black" />
                    </div>
                    <div >
                        <label class="block text-gray-700 text-sm font-bold mb-2">Email:</label>
                        <input v-model="userObj.Email" type="email" class="w-full border border-black" />
                    </div>
                    <div >
                ...
                <div class="flex justify-center" >
                    <button @click="SubmitData()" class="bg-green-500 text-white py-2 px-4 rounded mt-4 flex" title="Save">
                        Save
  ...
  export default {
    ...
    SubmitData() {
            ...
            if(!this.userObj.LRN_No||!this.userObj.SurName||!this.userObj.FirstName||
                !this.userObj.Phone_No||!this.userObj.Gender||!this.userObj.Track||
                !this.userObj.Year||!this.userObj.Section||!this.userObj.Email){
                    Swal.fire({
                        title: "Warning!",
                ...
                if(this.userObj.Phone_No.length !=11 ){
                    Swal.fire({
                        title: "Warning!",
                        text: `Mobile Number length must be 11!`,
                        icon: "warning"
                    });
                    return false
                }
                if(!this.userObj.Email.includes('@gmail.com') && !this.userObj.Email.includes('@yahoo.com')  ){
                    Swal.fire({
                        title: "Warning!",
      ...
      handlePhoneInput() {
        this.userObj.Phone_No = this.userObj.Phone_No.replace(/[^0-9]/g, '');

            if (this.userObj.Phone_No.length > 11) {
                this.userObj.Phone_No = this.userObj.Phone_No.slice(0, 11);
            }
        },
        getDateInfo(){
